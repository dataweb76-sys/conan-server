...
type StatusOk = {
  ...
  maxPlayers?: number | null; // ✅ number|null
  playersList?: string[]; // ✅ lista de nombres
};
...
export default function ServerModal({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // ✅ si el server no tiene port, no llamamos a nada
      if (typeof server.port !== "number") {
        setStatus({ state: "offline", error: "missing_port" });
        return;
      }
        ...
        <div className="grid gap-4 p-5 md:grid-cols-[1.2fr_0.8fr]">
              ...
              <div className="mt-3 grid gap-2 text-sm">
                ...
                {status.state === "ok" ? (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold">
                      {typeof status.data.players === "number" ? status.data.players : "—"}
                      {typeof status.data.maxPlayers === "number" ? `/${status.data.maxPlayers}` : ""}
                    </span>
                  </div>
                ) : status.state === "offline" ? (
                  <div className="mt-2 text-xs text-white/60">
                    {status.error === "missing_port"
                      ? "Falta el puerto en este server."
                      : "No responde ahora mismo (puede estar reiniciando)."}
                  </div>
                ) : null}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={() =>
                      alert(
                        `Para entrar: Conan Exiles → Direct Connect → ${connectString}\nQuery Port: ${
                          server.qport ?? "(no especificado)"
                        }`
                      )
                    }
                    className="rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-sm hover:bg-white/10"
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              {status.state === "ok" && Array.isArray(status.data.playersList) && status.data.playersList.length ? (
                <ul className="mt-3 space-y-1 text-sm text-white/80 max-h-64 overflow-auto">
                  {status.data.playersList.map((p) => (
                ...
                <div className="mt-3 text-xs text-white/60">
                  {status.state === "loading"
                    ? "Cargando…"
                    : "No hay lista de jugadores (o el server no la expone)."}
                </div>
              )}
            </div>
